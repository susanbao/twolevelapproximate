...
using namespace std;
const int maxc=3;
const int H=10;//the max search number
FILE* f_in;
FILE* f_out;
char s[100];
char u[50];
...
void MakeError(int reminderror,int deerror,int file) //file is name as XXYY, XX is the top error, YY=XX-deerror.
{
	if(deerror>=3)
		return;
	memset(s,0,sizeof(s));
...
void Begin(int reminderror)
{
	...
	MakeError(reminderror,1,1);
	if(reminderror>1)
	{
		excelsystem("cp b_0000.txt a_in.in");
		MakeError(reminderror,2,2);
	}
	for(int i=reminderror-1;i>0;i--)
	{
		for(int j=0;j<H;j++)
			if(er[i][j]!=0)
			{
				...
				MakeError(i,1,er[i][j]->file+(1*p3[num]));
				if(i>1)
				{
					MakeError(i,2,er[i][j]->file+(2*p3[num]));
				}		
			}
	}
	return;
...
int main(void)
{
	...
	for(int i=0;i<maxc;i++)
		if(mindl[i]<mindl[error])
			error=i;
	for(int i=0;i<maxc;i++)
		printf("When error is %d, the approximate literal is %d\n",i,mindl[i]);
	printf("the literal of file will be %d after approximate!\n",mindl[error]);
	printf("the decrease order is ");
	for(int i=0;i<maxc;i++)
		printf("%d",(minorder[error]/p3[i])%3);
	printf("\n");
	memset(s,0,sizeof(s));
	memset(u,0,sizeof(u));
	strcat(s,"cp r_");
